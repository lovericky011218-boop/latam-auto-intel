...
test("server-renders the expanded LATAM dashboard coverage", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>LATAM Auto Intel/);
  assert.match(html, /集团 × 市场覆盖密度/);
...
test("keeps expanded group data, filters, and official source links in source", async () => {
  ...
  assert.match(page, /七个市场暂未查到 VOYAH 官方在售目录/);
