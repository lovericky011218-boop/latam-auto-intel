...
const modelImage = (model:string) => {
  ...
  return `/cars/${slug}.${officialAvifModels.has(slug) ? "avif" : "jpg"}`;
};
...
const baseCars: BaseCar[] = Object.entries(raw).flatMap(([country, rows]) => rows.map((row, index) => {
  ...
  return {id:`${country}-${brand}-${model}-${index}`,country,flag:countries.find(x=>x[0]===country)?.[1]||"",group,brand,model,image:modelImage(model),price,trims,source,verified:auditedSources.has(source)?"2026-08-23":"2026-08-17",...(S[model]||{dims:"官网未披露",wheelbase:"官网未披露",energy:"待核验",use:"官网未披露",range:"官网未披露",safety:"未查到有效五星成绩",rating:"unknown"})};
}));
...
export default function Home(){
  ...
  return <main className="shell">
    ...
    <section className="brandFootprint" aria-label="全市场单一品牌车型投放规模"><div className="brandFootprintHead"><p>BRAND MODEL FOOTPRINT</p><h2>全市场 · 单一品牌车型投放规模</h2><span>统计全部战略市场内各品牌投放的独立车型数，不随上方大区选择变化。相同车型的多个动力形式只计 1 款；不同市场名称指向同一实际车型时合并计数。</span></div><div className="brandFootprintGrid">{brandFootprint.map(item=><button key={`${item.group}-${item.brand}`} title={item.models.join(" · ")} onClick={()=>{setRegion("全部区域");setCountry("全部市场");setGroup(item.group);setBrand(item.brand);setVisible(24);document.getElementById("lineup")?.scrollIntoView({behavior:"smooth"})}}><small>{groupLabels[item.group]} GROUP</small><b>{item.brand}</b><strong>{item.count}<em>款</em></strong><span>查看该品牌车型 →</span></button>)}</div><p className="brandFootprintNote">归一示例：Dolphin Mini / Dolphin Surf / Atto 1、Yuan Plus / Atto 3、Song Plus / Seal U / Sealion 6、Omoda 5 / E5、Dongfeng BOX / Nammi。</p></section>
