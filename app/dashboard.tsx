...
export default function Home(){
  ...
  const reset=()=>{setRegion("全部区域");setCountry("全部市场");setGroup("全部集团");setBrand("全部品牌");setEnergy("全部能源");setDrive("全部驱动");setSafe(false);setQuery("");setVisible(24);setOverviewBrand(null);setSelectedFamily(null);setFamilyFocus(null)};
  ...
  return <main className="shell">
    ...
    <section className="marketInsights" id="market-insights"><section className="coverage coverageTop" id="coverage"><div className="coverageTitle"><p>MARKET COVERAGE</p><h2>{coverageRegion} · 集团覆盖密度</h2><span>按大区切换市场；数字表示该集团在当地官方目录可识别的车型动力记录数。点击单元格可直接筛选。</span><div className="coverageRegions" aria-label="覆盖密度大区选择">{regionCountries.map(item=><button className={coverageRegion===item.name?"active":""} onClick={()=>setCoverageRegion(item.name)} key={item.name}><small>{item.code}</small>{item.name}<b>{item.countries.length}</b></button>)}</div></div><div className="coverageTable"><div className="covRow head"><span>市场</span>{groups.map(x=><span key={x}>{groupLabels[x]}</span>)}</div>{coverage.map(r=><div className="covRow" key={r.name}><strong>{r.flag} {r.name}</strong>{groups.map(g=><button onClick={()=>{setRegion(regionOfCountry(r.name));setCountry(r.name);setGroup(g);setBrand("全部品牌");document.getElementById("lineup")?.scrollIntoView({behavior:"smooth"})}} key={g}><b>{String((r as Record<string,unknown>)[g])}</b></button>)}</div>)}</div></section><section className="brandFootprint" aria-label="全市场单一品牌车型投放规模"><div className="brandFootprintHead"><p>BRAND MODEL FOOTPRINT</p><h2>全市场 · 单一品牌车型投放规模</h2><span>统计全部战略市场内各品牌投放的独立车型数，不随上方大区选择变化。相同车型的多个动力形式只计 1 款；不同市场名称指向同一实际车型时合并计数。</span></div><div className="brandFootprintGrid">{brandFootprint.map(item=><button key={`${item.group}-${item.brand}`} title={item.models.join(" · ")} onClick={()=>{setRegion("全部区域");setCountry("全部市场");setGroup(item.group);setBrand(item.brand);setEnergy("全部能源");setDrive("全部驱动");setSafe(false);setQuery("");setOverviewBrand(item.brand);setVisible(24)}}><small>{groupLabels[item.group]} GROUP</small><b>{item.brand}</b><strong>{item.count}<em>款</em></strong><span>查看该品牌车型 →</span></button>)}</div><p className="brandFootprintNote">归一示例：Dolphin Mini / Dolphin Surf / Atto 1、Yuan Plus / Atto 3、Song Plus / Seal U / Sealion 6、Omoda 5 / E5、Dongfeng BOX / Nammi；西班牙 EBRO s400 / s700 / s800 / s900 分别计入 Chery 对应车型族。</p></section></section>
