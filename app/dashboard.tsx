...
type BrandModelOverview = { key:string; name:string; image:string; powers:{name:string;configs:string[];total:number}[]; regions:string[]; countries:{name:string;flag:string}[]; aliases:string[]; records:Car[] };
...
const mpvModels = new Set(["Denza D9","M6","M6 DM-i","M7","Zeekr 009","VOYAH Dream","XPENG X9"]);
const pickupModels = new Set(["Shark","Himla","Poer","Poer P500","Wingle 5","Wingle 7","Rich 6","Rich 7","Hunter","F70","Z9"]);
const carModels = new Set(["Dolphin Mini","Dolphin","Dolphin Surf","Atto 1","King","Seal","Seal 6 DM-i","Seal 6 Touring","Arrizo 5 Pro","Arrizo 8 CSH","Emgrand","EX2","Ora 03","BOX","Nammi","E70","T03","Lumin","Eado Plus","XPENG P7+","NIO ET5","NIO ET5 Touring","firefly","B05","Zeekr 001"]);
...
export default function Home(){
  ...
  const filtered=useMemo(()=>{
    const matches=cars.filter(c=>{
      ...
      return (region==="全部区域"||regionOfCountry(c.country)===region)&&(country==="全部市场"||c.country===country)&&(group==="全部集团"||c.group===group)&&(brand==="全部品牌"||c.brand===brand)&&(body==="全部车身"||bodyTypeOf(c.model)===body)&&(energy==="全部能源"||energyKey(c.energy)===energy)&&(drive==="全部驱动"||c.drive.includes(drive))&&(!priceActive||(price!==null&&price>=priceMin&&price<=priceMax))&&(!safe||c.rating==="yes")&&(`${c.brand} ${c.model}`.toLowerCase().includes(query.toLowerCase()));
    });
  ...
  },[region,country,group,brand,body,energy,drive,priceActive,priceMin,priceMax,sortBy,safe,query]);
  ...
  const reset=()=>{setRegion("全部区域");setCountry("全部市场");setGroup("全部集团");setBrand("全部品牌");setBody("全部车身");setEnergy("全部能源");setDrive("全部驱动");setPriceMin(0);setPriceMax(priceCeiling);setSortBy("default");setSafe(false);setQuery("");setVisible(24);setOverviewBrand(null);setSelectedFamily(null);setFamilyFocus(null)};
  ...
  const overviewActive=Boolean(overviewBrand&&brand===overviewBrand&&region==="全部区域"&&country==="全部市场"&&body==="全部车身"&&energy==="全部能源"&&drive==="全部驱动"&&!priceActive&&sortBy==="default"&&!safe&&!query);
  useEffect(()=>{
  ...
  const brandModelOverview=useMemo<BrandModelOverview[]>(()=>{
    ...
    return [...map.entries()].map(([key,records])=>{
      ...
      return {key,name:overviewModelName(key),image:records[0].image,powers,regions:regionNames,countries:countryNames,aliases,records};
  ...
  return <main className="shell">
    ...
    <section className="marketInsights" id="market-insights"><section className="coverage coverageTop" id="coverage"><div className="coverageTitle"><p>MARKET COVERAGE</p><h2>{coverageRegion} · 集团覆盖密度</h2><span>按大区切换市场；数字表示该集团在当地官方目录可识别的车型动力记录数。点击单元格可直接筛选。</span><div className="coverageRegions" aria-label="覆盖密度大区选择">{regionCountries.map(item=><button className={coverageRegion===item.name?"active":""} onClick={()=>setCoverageRegion(item.name)} key={item.name}><small>{item.code}</small>{item.name}<b>{item.countries.length}</b></button>)}</div></div><div className="coverageTable"><div className="covRow head"><span>市场</span>{groups.map(x=><span key={x}>{groupLabels[x]}</span>)}</div>{coverage.map(r=><div className="covRow" key={r.name}><strong>{r.flag} {r.name}</strong>{groups.map(g=><button onClick={()=>{setRegion(regionOfCountry(r.name));setCountry(r.name);setGroup(g);setBrand("全部品牌");document.getElementById("lineup")?.scrollIntoView({behavior:"smooth"})}} key={g}><b>{String((r as Record<string,unknown>)[g])}</b></button>)}</div>)}</div></section><section className="brandFootprint" aria-label="全市场单一品牌车型投放规模"><div className="brandFootprintHead"><p>BRAND MODEL FOOTPRINT</p><h2>全市场 · 单一品牌车型投放规模</h2><span>统计全部战略市场内各品牌投放的独立车型数，不随上方大区选择变化。相同车型的多个动力形式只计 1 款；不同市场名称指向同一实际车型时合并计数。</span></div><div className="brandFootprintGrid">{brandFootprint.map(item=><button key={`${item.group}-${item.brand}`} title={item.models.join(" · ")} onClick={()=>{setRegion("全部区域");setCountry("全部市场");setGroup(item.group);setBrand(item.brand);setBody("全部车身");setEnergy("全部能源");setDrive("全部驱动");setPriceMin(0);setPriceMax(priceCeiling);setSortBy("default");setSafe(false);setQuery("");setOverviewBrand(item.brand);setVisible(24)}}><small>{groupLabels[item.group]} GROUP</small><b>{item.brand}</b><strong>{item.count}<em>款</em></strong><div className="bodyMix" aria-label={`${item.brand} 车身形式构成`}><span>轿车 <i>{item.bodyCounts["轿车"]}</i></span><span>SUV <i>{item.bodyCounts.SUV}</i></span><span>MPV <i>{item.bodyCounts.MPV}</i></span><span>皮卡 <i>{item.bodyCounts["皮卡"]}</i></span></div><span>查看该品牌车型 →</span></button>)}</div><p className="brandFootprintNote">车身形式与总车型数使用相同去重口径。归一示例：Dolphin Mini / Dolphin Surf / Atto 1、Yuan Plus / Atto 3、Song Plus / Seal U / Sealion 6、Omoda 5 / E5、Dongfeng BOX / Nammi；西班牙 EBRO s400 / s700 / s800 / s900 分别计入 Chery 对应车型族。</p></section></section>
    <section className="filterPanel" id="lineup">
      <div className="search"><span>⌕</span><input value={query} onChange={e=>{setQuery(e.target.value);setVisible(24)}} placeholder="搜索品牌或车型…"/><label className="sortControl"><small>排序</small><select value={sortBy} onChange={e=>{setSortBy(e.target.value as SortKey);setVisible(24)}}><option value="default">默认顺序</option><option value="priceAsc">价格：低 → 高</option><option value="priceDesc">价格：高 → 低</option><option value="rangeDesc">续航：高 → 低</option><option value="rangeAsc">续航：低 → 高</option><option value="verifiedDesc">资料更新时间：新 → 旧</option><option value="nameAsc">品牌 / 车型：A → Z</option><option value="launch" disabled>上市时间（待补齐）</option></select></label><kbd>{overviewActive?`${brandModelOverview.length} 款车型`:`${filtered.length} 条结果`}</kbd></div>
